...
import com.westalgo.factorycamera.util.Size;

/**
 * SettingsManager class provides an api for getting and setting
 ...
 *
 * This also enables the user to safely get settings values as three different
 * types, as it's convenient: String, Integer, and boolean values. Integers and
 * boolean can always be trivially converted to one another, but Strings cannot
 * always be parsed as Integers. In this case, if the user stores a String value
 * that cannot be parsed to an Integer yet they try to retrieve it as an
 * Integer, the API throws a meaningful exception to the user.
 *
 * Scope
 *
 * This API introduces the concept of "scope" for a setting, which is the
 * generality of a setting. The most general settings, that can be accessed
 * acrossed the entire application, have a scope of SCOPE_GLOBAL. They are
 * stored in the default SharedPreferences file.
 *
 * A setting that is local to a third party module or subset of the application
 * has a custom scope. The specific module can define whatever scope (String)
 * argument they want, and the settings saved with that scope can only be seen
 * by that third party module. Scope is a general concept that helps protect
 * settings values from being clobbered in different contexts.
 *
 * Keys and Defaults
 *
 * This API allows you to store your SharedPreferences keys and default values
 * outside the SettingsManager, because these values are either passed into the
 * API or stored in a cache when the user sets defaults.
 *
...
public class SettingsManager {
    private static final Log.Tag TAG = new Log.Tag("SettingManager");
    private Context mContext;

    // One global SharedPreferences. Multiple custom SharedPreferences
    private final SharedPreferences mGlobalPreferences;
    private SharedPreferences mCustomPreferences;
    private String mCustomScope = null;

    private final DefaultsStore mDefaultsStore = new DefaultsStore();
    private List<OnSettingChangedListener> mSettingListeners = new ArrayList<OnSettingChangedListener>();
    private List<OnSharedPreferenceChangeListener> mSharedPreferenceListeners = new ArrayList<OnSharedPreferenceChangeListener>();

    public SettingsManager(Context context) {
        mContext = context;
        mGlobalPreferences = PreferenceManager
                             .getDefaultSharedPreferences(mContext);
    }

    //======================================================================================
    /**
     * Manager OnSharedPreferenceChangeListener for Global Preferences and Custom Preferences
     */
    //======================================================================================
    ...
    /**
     * Open a SharedPreferences file by custom scope. Also registers any known
     * SharedPreferenceListeners on this SharedPreferences instance.
     */
    protected SharedPreferences openPreferences(String scope) {
        SharedPreferences preference = mContext.getSharedPreferences(
                                           mContext.getPackageName() + scope, Context.MODE_PRIVATE);
    ...
    /**
     * Close a SharedPreferences file by custom scope. The file isn't explicitly
     * closed (the SharedPreferences API makes this unnecessary), so the real
     * work is to unregister any known SharedPreferenceListeners from this
     * SharedPreferences instance.
     *
     * It's important to do this as camera and modules change, because we don't
     * want old SharedPreferences listeners executing on cameras/modules they
    ...
    public void addListener(final OnSettingChangedListener listener) {
        ...
        if (mSettingListeners.contains(listener)) {
            return;
        }
        ...
        //Log.v(TAG, "listeners: " + mSettingListeners);
    }
    ...
    public void removeListener(OnSettingChangedListener listener) {
        ...
        int index = mSettingListeners.indexOf(listener);
        mSettingListeners.remove(listener);

        OnSharedPreferenceChangeListener sharedPreferenceListener = mSharedPreferenceListeners
                .get(index);
    ...
    /**
     * Returns the SharedPreferences file matching the scope argument.
     *
     * Camera and module preferences files are cached, until the camera id or
     * module id changes, then the listeners are unregistered and a new file is
     * opened.
     */
    private SharedPreferences getPreferencesFromScope(String scope) {
        ...
        if (mCustomPreferences != null && mCustomScope != null
                && mCustomScope.equals(scope)) {
    ...
    //======================================================================================
    /**
     * When open Camera, mDefaultsStore would save all possible value response for key.
     *
     * Get value from DefaultsStore.
     */
    //===================================================================================
    /**
     * Set default and valid values for a setting, for a String default and a
     * set of String possible values that are already defined. This is not
     * required.
     */
    public void setDefaults(String key, String defaultValue,
                            String[] possibleValues) {
        mDefaultsStore.storeDefaults(key, defaultValue, possibleValues);
    }

    /**
     * Set default and valid values for a setting, for an Integer default and a
     * set of Integer possible values that are already defined. This is not
     * required.
     */
    public void setDefaults(String key, int defaultValue, int[] possibleValues) {
        ...
        mDefaultsStore.storeDefaults(key, defaultValueString,
                                     possibleValuesString);
    }

    /**
     * Set default and valid values for a setting, for a boolean default. The
     * set of boolean possible values is always { false, true }. This is not
     * required.
     */
    ...
    public Integer getIntegerDefault(String key) {
        ...
        return defaultValueString == null ? 0 : Integer
    ...
    public boolean getBooleanDefault(String key) {
        String defaultValueString = mDefaultsStore.getDefaultValue(key);
        return defaultValueString == null ? false : (Integer
                .parseInt(defaultValueString) != 0);
    }

    //======================================================================================
    /**
     * Get key-value from Global/Custom SharedPreferences
     */
    //======================================================================================
    /**
     * Retrieve a setting's value as a String, manually specifiying a default
     * value.
     */
    public String getString(String scope, String key, String defaultValue) {
        ...
        try {
            ...
            Log.w(TAG,
                  "existing preference with invalid type, removing and returning default",
                  e);
    ...
    /**
     * Retrieve a setting's value as a String, using the default value stored in
     * the DefaultsStore.
     */
    ...
    /**
     * Retrieve a setting's value as an Integer, manually specifying a default
     * value.
     */
    ...
    /**
     * Retrieve a setting's value as a boolean, manually specifiying a default
     * value.
     */
    ...
    /**
     * If possible values are stored for this key, return the index into that
     * list of the currently set value.
     *
     * For example, if a set of possible values is [2,3,5], and the current
     * value set of this key is 3, this method returns 1.
     *
     * If possible values are not stored for this key, throw an
    ...
    public int getIndexOfCurrentValue(String scope, String key) {
        ...
        if (possibleValues == null || possibleValues.length == 0) {
            throw new IllegalArgumentException("No possible values for scope="
                                               + scope + " key=" + key);
        }
        ...
        throw new IllegalStateException("Current value for scope=" + scope
                                        + " key=" + key + " not in list of possible values");
    }


    //======================================================================================
    /**
     * set key-value to Global/Custom SharedPreferences
     */
    //======================================================================================
    /**
     * Store a setting's value using a String value. No conversion occurs before
    ...
    /**
     * Store a setting's value using an Integer value. Type conversion to String
    ...
    /**
     * Store a setting's value using a boolean value. Type conversion to an
     * Integer and then to a String occurs before this value is stored in
     * SharedPreferences.
     */
    ...
    /**
     * If a set of possible values is defined, set the current value of a
     * setting to the possible value found at the given index.
     *
     * For example, if the possible values for a key are [2,3,5], and the index
     * given to this method is 2, then this method would store the value 5 in
     * SharedPreferences for the key.
     *
     * If the index is out of the bounds of the range of possible values, or
     * there are no possible values for this key, then this method throws an
     * exception.
     */
    public void setValueByIndex(String scope, String key, int index) {
        ...
        if (index >= 0 && index < possibleValues.length) {
            ...
            throw new IndexOutOfBoundsException("For possible values of scope="
                                                + scope + " key=" + key);
        }
    }
