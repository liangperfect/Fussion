...
//    private CameraAppUI.CameraModuleScreenShotProvider mScreenShotProvider = null;
    ...
    private class FullyHiddenState extends ModeListState {
        ...
        @Override
        public boolean onTouchEvent(MotionEvent ev) {
//            Log.e(TAG,"FullyHiddenState onTouchEvent");
        ...
        @Override
        public boolean shouldHandleVisibilityChange(int visibility) {
//            Log.e(TAG," FullyHiddenState shouldHandleVisibilityChange");
            if (mAnimator != null) {
                return false;
            }
...
//            Log.e(TAG,"snapOpenAndShow");
    ...
    private class FullyShownState extends ModeListState {
        ...
        @Override
        public boolean onTouchEvent(MotionEvent ev) {
//            Log.e(TAG,"FullyShownState onTouchEvent");
            if (ev.getActionMasked() == MotionEvent.ACTION_DOWN) {
    ...
    private class ShimmyState extends ModeListState {
        ...
        @Override
        public boolean onTouchEvent(MotionEvent ev) {
//           Log.e(TAG," ShimmyState onTouchEvent");
            if (MotionEvent.ACTION_DOWN == ev.getActionMasked()) {
                if (ev.getActionMasked() == MotionEvent.ACTION_DOWN) {
                    mFocusItem = getFocusItem(ev.getX(), ev.getY());
                    setSwipeMode(true);
    ...
    private class ScrollingState extends ModeListState {
        ...
        @Override
        public boolean onTouchEvent(MotionEvent ev) {
            //Log.e(TAG,"ScrollingState onTouchEvent");
...
//                        .getPreviewFrame(PREVIEW_DOWN_SAMPLE_FACTOR),getFullscreenRect());
...
//            }
    ...
    public void init(List<Integer> modeIndexList) {
        ...
        Log.d(TAG, "------>>init mTotalModes = " + mTotalModes);
        initializeModeSelectorItems();
        Log.d(TAG, "------>>init findViewById ");
...
//    }
    ...
    private void initializeModeSelectorItems() {
        ...
        resetModeSelectors();
        Log.d(TAG, "------>>initializeModeSelectorItems end");
    }
...
//                mTotalModes);
        return getContext().getResources().getInteger(R.integer.camera_mode_photo);
    }
    ...
    private void forwardTouchEventToChild(MotionEvent ev) {
        if (mChildViewTouched != null) {
            //Log.e(TAG,"forwardTouchEventToChild");
            float x = ev.getX() - mListView.getX();
            float y = ev.getY() - mListView.getY();
...
//        Log.e(TAG," updateModeListLayout");
        // Center mode drawer in the portion of camera preview that is not covered by
        // bottom bar.
        Rect uncoveredPreviewArea  = mCameraAppUI.getPreviewRect();
        // Align left:
    ...
    private void updateSettingsButtonLayout(Rect uncoveredPreviewArea) {
        Log.v(TAG,"----- updateSettingsButtonLayout mWidth:" + mWidth + ":mHeight:" + mHeight);
        Log.v(TAG,"----- updateSettingsButtonLayout uncoveredPreviewArea:" + uncoveredPreviewArea.toString());
        if (mWidth > mHeight) {
...
//        Log.e(TAG, "Invalid time input for getPosition(). time: " + time);
    ...
    private class PeepholeAnimationEffect extends AnimationEffects {
        ...
        public PeepholeAnimationEffect() {
            Log.e(TAG,"PeepholeAnimationEffect");
