...
public class Storage {
    ...
    private static final Log.Tag TAG = new Log.Tag(STORAGE_STRING_ID);
    ...
    /**
     * image type:yuv  raw  jpeg
     */
    public static final String IMAGE_TYPE_YUV = "yuv";
    //Only for save in DEBUG_DIRECTORY
    public static final String IMAGE_TYPE_DEBUGYUV = "debug_yuv";
    ...
    public static int writeFile(String path, byte[] jpeg, ExifInterface exif,
                                String mimeType) {
        if (exif != null
                && (mimeType == null || mimeType.equalsIgnoreCase(IMAGE_TYPE_JPEG))) {
            try {
            ...
            if (!(mimeType.equalsIgnoreCase(IMAGE_TYPE_JPEG) || mimeType == null)) {
    ...
    public static Uri addImage(ContentResolver resolver, String title,
                               ...
                               byte[] data, int width, int height, String mimeType) {
        ...
        if(mimeType.equalsIgnoreCase(IMAGE_TYPE_YUV) || mimeType.equalsIgnoreCase(IMAGE_TYPE_DEBUGYUV)) {
            return addYuvImage(resolver, title, date, location, orientation, exif,
                    data, width, height, mimeType);
        }
        String path = generateFilepath(title, mimeType);
        int size = writeFile(path, data, exif, mimeType);


        Log.v(TAG, "save jpeg time:" + (System.currentTimeMillis() - startTime));
        return addImage(resolver, title, date, location, orientation, size,
                        path, width, height, mimeType);
    }

    // Save yuv image adn add it to MediaStore.
    public static Uri addYuvImage(ContentResolver resolver, String title,
                                  long date, Location loc, int orientation, ExifInterface exif,
                                  byte[] data, int width, int height, String mimeType) {
        String path = null;
        if(mimeType.equalsIgnoreCase(IMAGE_TYPE_YUV)){
            path = DIRECTORY + '/' + title + JPEG_POSTFIX;
        }else{
            path = DEBUG_DIRECTORY + '/' + title + JPEG_POSTFIX;
        }
        ...
        if (image != null) {
            try {
                if (exif != null) {
                    ...
                    int size = writeFile(path, jpeg, exif, IMAGE_TYPE_JPEG);
                    baos.close();
                    baos = null;
                    return addImage(resolver, title, date, loc, orientation, size, path, width,
                            height, IMAGE_TYPE_JPEG);
                } else {
                    ...
                    stream.flush();
                    stream.close();
                    return addImage(resolver, title, date, loc, orientation, size,
                            path, width, height, IMAGE_TYPE_JPEG);
                }
            } catch (/*IOException*/Exception e) {
                Log.e(TAG, "addYuvImage", e);
            } finally {
                image = null;
            }
        }
    ...
    public static ContentValues getContentValuesForData(String title,
        ...
        if (mimeType.equalsIgnoreCase("jpeg")
                || mimeType.equalsIgnoreCase("image/jpeg") || mimeType == null) {
    ...
    public static void updateImage(Uri imageUri, ContentResolver resolver,
                                   String title, long date, Location location, int orientation,
                                   ExifInterface exif, byte[] jpeg, int width, int height,
                                   String mimeType) {
        String path = generateFilepath(title, mimeType);
        writeFile(path, jpeg, exif, mimeType);
        updateImage(imageUri, resolver, title, date, location, orientation,
    ...
    public static void updateImage(Uri imageUri, ContentResolver resolver,
                                   String title, long date, Location location, int orientation,
                                   int jpegLength, String path, int width, int height, String mimeType) {
        ...
        if (rowsModified == 0) {
            ...
            Log.w(TAG, "updateImage called with no prior image at uri: "
                  + imageUri);
            insertImage(resolver, values);
        } else if (rowsModified != 1) {
            // This should never happen
            throw new IllegalStateException("Bad number of rows ("
                                            + rowsModified + ") updated for uri: " + imageUri);
        }
    }
    ...
    public static String generateFilepath(String title, String pictureFormat) {
        if (pictureFormat == null || pictureFormat.equalsIgnoreCase(IMAGE_TYPE_JPEG)) {
    ...
    private static Uri insertImage(ContentResolver resolver,
